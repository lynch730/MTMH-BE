...
int main() {
	...
	double EN0 = 10.0;
	int Nu_grid[kmax] = {50, 100, 150, 200, 250, 300, 350, 400, 450, 500, 550, 600, \
			    650, 700, 750, 800, 850, 900, 950, 1000, 4000};
	...
	// Erase Species - Add Cross Sections 
	int imaxs = lib0.allspecies.size()-1;
	for (int i = imaxs; i >= ispec; --i) {
		lib0.erase_by_index(i);
	}
	...
	for (int k = 0; k < kmax; ++k) {
		...
		for (int it = 0; it < N_trial; ++it) {
			...
			for (int i = 0; i < ispec; ++i) {
				...
				lib0.assign_fracs_by_names(sname, sfrac[i]);
				//mb::extra_normal_statement(std::to_string(i) + ",  " + sname + ",  " + mb::mb_format(sfrac[i]) );
			}

			// Run Solver
			mb::BoltzmannSolver solver = mb::BoltzmannSolver(p, lib0);
