...
module mdat
    ...
    ! Real Arrays
    real(8), allocatable, dimension(:) :: grid_EC, grid_OC, C_Vw, zin_grat, zin_de, &
...
contains
    ...
    subroutine load_vars(fname)
        ...
        call mat2int(mp, 'jac_Iu', A_mp)
        call mat2int(mp, 'jac_Ju', A_np)
        call mat2int(mp, 'Y_I', C_pw)
        call mat2int(mp, 'Y_J', C_zw)
    ...
    subroutine destroy_vars()
        ...
        if (allocated(A_mp)) deallocate(A_mp)
        if (allocated(A_np)) deallocate(A_np)
        if (allocated(C_pw)) deallocate(C_pw)
        if (allocated(C_zw)) deallocate(C_zw)
        ...
        if (allocated(C_Vw)) deallocate(C_Vw)   
